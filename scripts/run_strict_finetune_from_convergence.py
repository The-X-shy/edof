"""Build the selected exact 40x40 PSF map and run the paper-loss fine-tune."""
...
from edof_reproduction.convergence import build_strict_finetune_config
...
def run(project_root: Path) -> dict[str, Any]:
    ...
    output = workspace / "windows_strict_finetune"
    ...
    try:
        ...
        write_json(state_path, state)
        print(json.dumps({"strict_finetune_result": result}, ensure_ascii=False), flush=True)
        return result
