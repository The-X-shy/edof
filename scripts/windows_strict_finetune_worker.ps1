...
$Runner = Join-Path $ProjectRoot "scripts\run_strict_finetune_from_convergence.py"
...
$Output = Join-Path $ProjectRoot "workspace\edof_reproduction\windows_strict_finetune"
$Stdout = Join-Path $Output "strict_finetune.stdout.log"
$Stderr = Join-Path $Output "strict_finetune.stderr.log"
$ExitStatus = Join-Path $Output "strict_finetune.exit.json"
...
if (-not (Test-Path $Runner)) {
    throw "Strict fine-tune runner is missing: $Runner"
}
